...
import com.niuniu.babyprotect.websocket.ScreenAndBattaryStatusEvent;
import java.util.Timer;
import java.util.TimerTask;
public class ScreenReceiver extends BroadcastReceiver {
    public static final String ACTION_ACCESSIBILITY_START = "action_app_accessibility_start";
    public static final String ACTION_ACCESSIBILITY_STOP = "action_app_accessibility_stop";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        } else if (this.action.equals(ACTION_ACCESSIBILITY_START)) {
            ILog.d(TAG, "ACTION_ACCESSIBILITY_START");
            WebSocketManager.getInstance().reconnect();
            WebSocketManager.getInstance().sendScreenEventMessage("1013", "自动辅助权限已经开启");
        } else if (this.action.equals(ACTION_ACCESSIBILITY_STOP)) {
    ...
    private int getBatteryLevel(Context context) {
        BatteryManager manager = (BatteryManager) context.getSystemService("batterymanager");
