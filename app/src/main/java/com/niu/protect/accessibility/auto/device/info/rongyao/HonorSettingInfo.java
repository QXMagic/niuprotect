package com.niu.protect.accessibility.auto.device.info.rongyao;
...
public class HonorSettingInfo extends BaseCreatePageInfo {
    @Override
    protected BaseCreatePageInfo createInfo() {
        return new HonorSettingInfo();
    }
    ...
    public List<PageInfoModel> createPage(Context context) {
        ...
        PageInfoModel pInfoAutoStartCenter = createLastPage("应用启动管理", 1, 3, 0, "应用自启动管理", 10);
        pInfoAutoStartCenter.addCheckBox(createCheckBoxStep(Constant.APP_NAME, 0, true));
        childinfoModels.add(pInfoAutoStartCenter);
        PageInfoModel pInfoPowerDetail = createLastAlterWindowsPage("手动管理", 1, 4, "确定", 3, "应用自启动管理", 10);
        ...
        PageInfoModel pInfoPremCenter = createLastPage(Constant.APP_NAME, 1, 7, 2, "通知管理", 2);
        ...
        PageInfoModel pInfoFloatViewCenter = createParentPage("显示在其他应用的上层", Constant.APP_NAME, 3, 9);
        ...
        PageInfoModel pInfoCenterAPPUse = createCenterPage("使用情况", Constant.APP_NAME, 4, 12);
